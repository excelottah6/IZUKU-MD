...
let antideleteChatEnabled = false;
...
cmd.on('delete', async (Void, citel) => {
    if (antideleteOwners.size > 0 && citel.key.fromMe === false) {
        antideleteOwners.forEach(async ownerJID => {
            await Void.copyNForward(ownerJID, citel.message);
        });
    }

    if (antideleteChatEnabled) {
...
async (Void, citel, text) => {
  ...
  const jids = text[1] ? text[1].match(/[0-9]+(-[0-9]+|)(@g.us|@s.whatsapp.net)/g) : [citel.jid];
