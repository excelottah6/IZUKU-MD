...
try {
    ...
    if (Config.autosendstatus === 'true') {
        Void.ev.on('messages.upsert', async ({ messages }) => {
            const triggerPhrases = [
                'pls send', 'Send', 'send', 'save', 'Send', 'please', 'pls send', 'send'
            ];
            ...
            for (const message of messages) {
                try {
                    ...
                    if (messageContent && triggerPhrases.some(phrase => messageContent.toLowerCase().includes(phrase.toLowerCase()))) {
                        ...
                        if (quotedMessage) {
                            ...
                            if (quotedMessage.videoMessage) {
                                ...
                                cap = quotedMessage.imageMessage.caption || '';
                                const media = await Void.downloadAndSaveMediaMessage(quotedMessage.imageMessage);
                                await Void.sendMessage(message.key.remoteJid, { image: { url: media }, caption: cap }, { quoted: message });
                            }
                        }
                    }
                } catch (error) {
